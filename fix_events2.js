...
const replacement = `function EventCard({ event, index, isActive, isHovered, onHover, onLeave, expanded, onToggleExpand }) {
  ...
  const inView = framerMotion.useInView(ref, { once: true, amount: 0.15 });
...
try {
  ...
  if (startIndex !== -1 && endIndex !== -1) {
    ...
    // Also fix React.useRef and framerMotion.useInView since we need to just use useRef and useInView directly
    const finalContent = newContent
      .replace('const ref = React.useRef(null);', 'const ref = useRef(null);')
      .replace('const inView = framerMotion.useInView(ref', 'const inView = useInView(ref');
      
    fs.writeFileSync('c:/anubhudhi foundation/Anubhuthi_foundation_FE/src/pages/EventsPage.jsx', finalContent, 'utf8');
    console.log('Successfully applied new EventCard design.');
  } else {

...
const replacement = `function EventCard({ event, index, isActive, isHovered, onHover, onLeave, expanded, onToggleExpand }) {
  ...
  return (
    <>
      ...
      >
        <div
          className={\`relative rounded-2xl transition-all duration-400 overflow-hidden flex flex-col md:justify-end bg-[#010D1E] min-h-[auto] md:min-h-[420px] \${
            isHovered || expanded 
        ...
        >
            ...
            {expanded && (
              ...
              >
                <div className="p-6 space-y-4">
                  {event.description && (
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: '#D4A84F' }}>About</p>
                      <p className="text-sm text-white/80 leading-relaxed whitespace-pre-line">{event.description}</p>
                    </div>
                  )}
                  {event.venue && (
                    <div>
          ...
          <div className="relative z-20 w-full bg-[#010D1E]/95 backdrop-blur-md border-t border-white/10 px-6 py-3 flex items-center justify-between mt-auto">
            <button
              onClick={(e) => {
                e.stopPropagation();
                onToggleExpand();
      ...
      <AnimatePresence>
        {showImageModal && (
          <motion.div
            ...
            className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-8 bg-[#010D1E]/95 backdrop-blur-lg"
          >
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="absolute top-4 right-4 sm:top-8 sm:right-8 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors border border-white/20 shadow-lg z-10"
            >
...
try {
  ...
  // Update imports for FaSearchPlus
  if (!content.includes('FaSearchPlus')) {
    content = content.replace('FaCheckCircle } from', 'FaCheckCircle, FaSearchPlus } from');
  }
  ...
  if (startIndex !== -1 && endIndex !== -1) {
    ...
    console.log('Successfully applied responsive EventCard with lightbox.');
  } else {
